...
import {
  ...
  QUICK_PAY_AMOUNTS,
  QUICK_QTY,
  ROOM_CHARGE_SERVICE_TYPES,
  SECONDARY_QUEUE_VIEWS,
  TABLE_SHAPES,
  TENDER_ACCOUNT_HINTS,
  TENDER_LABELS,
  TENDERS,
  calculateCartTotals,
  elapsedLabel,
...
export default function PosPage() {
  ...
  const [mapDraftBaseline, setMapDraftBaseline] = useState(null);
  ...
  const [paidOut, setPaidOut] = useState({ open: false, amount: '', category: '', note: '', reference_no: '' });
  ...
  const nativeDialogKey = assignTablePrompt.open ? 'assign-table'
    : availabilityOpen ? 'availability'
      : mapManagerOpen ? 'service-map'
        : tableAction.open ? 'table-action'
          : lineEditor.open ? 'line-editor'
            : paymentOpen ? 'payment'
              : receiptPreview ? 'receipt'
                : closedSessionPacket ? 'close-packet'
                  : variantPicker.open ? 'variant-picker'
                    : moneyDrop.open ? 'money-drop'
                      : paidOut.open ? 'paid-out'
                        : quickClose.open ? 'quick-close'
                          : configurator.open ? 'configurator' : '';
  useDialogFocus(!!nativeDialogKey, closeNativeDialogs, nativeDialogKey);

  function closeNativeDialogs() {
    setAssignTablePrompt((prev) => ({ ...prev, open: false }));
    setAvailabilityOpen(false);
    if (mapManagerOpen && mapDraftBaseline) setTableLayout(mapDraftBaseline);
    setMapDraftBaseline(null);
    setMapManagerOpen(false);
    setTableAction((prev) => ({ ...prev, open: false }));
    setLineEditor((prev) => ({ ...prev, open: false }));
    setPaymentOpen(false);
    setReceiptPreview(null);
    setClosedSessionPacket(null);
    setVariantPicker((prev) => ({ ...prev, open: false }));
    setMoneyDrop((prev) => ({ ...prev, open: false }));
    setPaidOut((prev) => ({ ...prev, open: false }));
    setQuickClose((prev) => ({ ...prev, open: false }));
    setConfigurator((prev) => ({ ...prev, open: false }));
  }
  ...
  useEffect(() => {
    fetchReceiptProfile().then((profile) => setReceiptProfile(profile || {})).catch(() => setReceiptProfile({}));
  }, []);
  useEffect(() => {
    const registerId = sessions.find((row) => String(row.id) === String(sessionId))?.register_id;
    setLastReceipt(loadLastReceipt({ ownerId: user?.id, registerId }));
    setOfflineDrafts(listOfflineDrafts({ ownerId: user?.id, registerId }));
  }, [sessionId, sessions, user?.id]);
  useEffect(() => {
  ...
  const currentSessionIsStale = !!currentSession && (currentSession.is_stale || currentSession.business_date !== todayISO());
  const moneyDropAccounts = useMemo(() => accountingAccounts.filter((row) => ['safe', 'bank'].includes(String(row.account_type || row.type || '').toLowerCase())), [accountingAccounts]);
  const selectedMoneyDropAccount = useMemo(() => moneyDropAccounts.find((row) => String(row.id) === String(moneyDrop.to_accounting_financial_account_id)) || null, [moneyDropAccounts, moneyDrop.to_accounting_financial_account_id]);
  ...
  function openCustomerDisplay() { closeToolsMenu(); window.open('/customer-display?channel=main', '_blank', 'width=900,height=700'); }
  function openMapManager() {
    closeToolsMenu();
    setMapDraftBaseline(structuredClone(tableLayout));
    setMapManagerOpen(true);
    setMapEditor((prev) => ({ ...prev, editing: true, draggingId: '' }));
  }
  function discardMapLayout() {
    if (mapDraftBaseline) setTableLayout(mapDraftBaseline);
    setMapDraftBaseline(null);
    setMapManagerOpen(false);
  ...
  function refreshOfflineDrafts() {
    setOfflineDrafts(listOfflineDrafts({ ownerId: user?.id, registerId: currentSession?.register_id }));
  }
  function saveCurrentOfflineDraft(reason = 'manual') {
    ...
    const draft = saveOfflineDraft(buildOfflineDraftSnapshot(reason), { ownerId: user?.id, ownerLabel: user?.display_name || user?.username || '', registerId: currentSession?.register_id || null });
  ...
  function restoreOfflineDraft(draft) {
    if (!draft) return;
    if (draft.ownership_mismatch && !window.confirm('This draft was saved by another operator or register. Restore it only after confirming ownership and handover.')) return;
  ...
  async function handlePay() {
    ...
    try {
      ...
      const receipt = { ...settled, receipt_profile: receiptProfile };
      setReceiptPreview(receipt); saveLastReceipt(receipt, { ownerId: user?.id, registerId: currentSession?.register_id }); setLastReceipt(receipt); setPaymentOpen(false); setNotice(settled.status === 'folio_pending' ? `Order ${order.order_no} finalized to pending folio.` : `Order ${order.order_no} paid.`); clearCart(); setTerminalScreen('spaces'); await loadAll({ silent: true });
  ...
  async function handleQuickClose() {
    ...
    if (!String(quickClose.sign_off_name || '').trim() || !String(quickClose.sign_off_role || '').trim()) return setError('Enter the sign-off name and role before closing the session.');
    try {
  ...
  function openMoneyDrop() {
    if (!currentSession?.id) return setError('Select an open session first.');
    closeToolsMenu();
    setMoneyDrop({ open: true, amount: '', to_accounting_financial_account_id: '', note: '', reference_no: '' });
  }
  ...
  function openPaidOut() {
    if (!currentSession?.id) return setError('Select an open session first.');
    closeToolsMenu();
    setPaidOut({ open: true, amount: '', category: '', note: '', reference_no: '' });
  }
  async function handlePaidOut() {
    setError(''); setNotice('');
    if (!currentSession?.id) return setError('Select an open session first.');
    if (num(paidOut.amount, 0) <= 0) return setError('Paid out amount must be greater than zero.');
    if (!String(paidOut.category || '').trim()) return setError('Select a paid out category.');
    if (!String(paidOut.reference_no || '').trim()) return setError('Enter the receipt or reference number for this paid out.');
    try {
      await createCashMovement({
        register_session_id: Number(currentSession.id),
        direction: 'out',
      ...
      setPaidOut({ open: false, amount: '', category: '', note: '', reference_no: '' });
      await loadAll({ silent: true });
    } catch (e) {
  ...
  function duplicateSelectedMapItem() {
    if (!selectedMapTable) return;
    const copy = {
      ...selectedMapTable,
      id: `${isMapVisualElement(selectedMapTable) ? 'element' : 'table'}-${Date.now()}`,
      code: `${selectedMapTable.code || 'Item'} Copy`,
      x: Math.min(94, num(selectedMapTable.x, 50) + 4),
      y: Math.min(90, num(selectedMapTable.y, 50) + 4),
    };
    setTableLayout((prev) => ({ ...prev, tables: [...(prev.tables || []), copy] }));
    setMapEditor((prev) => ({ ...prev, selectedId: copy.id }));
  }
  function nudgeSelectedMapItem(dx, dy) {
    if (!selectedMapTable) return;
    updateMapTable(selectedMapTable.id, {
      x: Math.max(6, Math.min(94, num(selectedMapTable.x, 50) + dx)),
      y: Math.max(10, Math.min(90, num(selectedMapTable.y, 50) + dy)),
    });
  }
  function handleMapItemKeyDown(event) {
    const deltas = { ArrowLeft: [-1, 0], ArrowRight: [1, 0], ArrowUp: [0, -1], ArrowDown: [0, 1] };
    if (deltas[event.key]) {
      event.preventDefault();
      nudgeSelectedMapItem(...deltas[event.key]);
    }
  }
  ...
  function loadOrderIntoCart(order, shouldResume = true) {
    ...
    const after = (nextOrder) => {
      ...
      setCart((nextOrder.lines || []).map((line) => {
        const catalogItem = catalog.find((item) => String(item.id) === String(line.catalog_item_id));
        return recalcLine({ local_id: crypto.randomUUID(), catalog_item_id: line.catalog_item_id, name: line.item_name_snapshot, customer_display_name: line.item_name_snapshot, sku_code: line.sku_code, base_price: num(line.unit_price), price: num(line.unit_price), tax_rate: num(catalogItem?.tax_rate), service_charge_rate: num(catalogItem?.service_charge_rate), quantity: num(line.quantity), note: line.note || '', manual_discount_amount: num(line.discount_amount), promo_discount_amount: 0, discount_amount: num(line.discount_amount) });
      }));
    ...
    <div className="stack pos-page-root">
      ...
      <section className="section pos-terminal-top">
        <div className="pos-compact-topbar">
          ...
          <div className="pos-status-cluster" aria-label="Terminal status">
            <span className={`badge ${currentSessionIsStale ? 'danger' : currentSession ? 'success' : 'warn'}`}>
              {currentSessionIsStale ? 'Stale session' : currentSession ? currentSession.session_code : 'No session'}
            </span>
            {currentOrderId && (
          ...
          <div className="pos-top-actions">
            <details className="pos-tools-menu" ref={toolsMenuRef}>
              <summary>Tools</summary>
              <div className="pos-tools-panel">
                {currentSession && <div className="pos-tools-group-label">Shift and cash</div>}
                {currentSession && (
                ...
                <div className="pos-tools-group-label">Menu and layout</div>
                <button type="button" className="secondary" onClick={() => { closeToolsMenu(); setAvailabilityOpen(true); }}>Menu Availability</button>
                <button type="button" className="secondary" onClick={openMapManager}>Manage Service Map</button>
                <div className="pos-tools-group-label">Display and utility</div>
                <button type="button" className="secondary" onClick={openCustomerDisplay}>Open Customer Screen</button>
                <button type="button" className="secondary" onClick={() => { closeToolsMenu(); requestFullScreen(); }}>Full Screen</button>
                {renderOfflineDraftTools()}
                {!!lastReceipt && (
                  <button type="button" className="secondary" onClick={() => printReceipt({ ...lastReceipt, receipt_profile: receiptProfile })}>Reprint Last Receipt</button>
                )}
              </div>
            </details>
      ...
      {sessionsStatus === 'loading' && <section className="section pos-session-state" aria-live="polite" aria-busy="true"><h2>Loading register sessions…</h2><p className="muted">Checking the current drawer and shift before opening the terminal.</p></section>}
      {sessionsStatus === 'error' && !sessions.length && <section className="section pos-session-state" role="alert"><h2>Could not load register sessions</h2><p className="muted">The terminal will not offer a new session until it confirms the current drawer state.</p><button type="button" className="primary" onClick={() => loadAll()}>Retry</button></section>}
      {sessionsStatus === 'loaded' && !sessions.length && <section className="section"><h2>Open a Session</h2><form className="form-grid" onSubmit={handleOpenSession} style={{ marginTop: 12 }}><label className="field">Register<select value={openSessionForm.register_id} onChange={(e) => setOpenSessionForm((prev) => ({ ...prev, register_id: e.target.value }))}><option value="">Select register</option>{registers.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label><label className="field">Business Date<input type="date" value={openSessionForm.business_date} onChange={(e) => setOpenSessionForm((prev) => ({ ...prev, business_date: e.target.value }))} /></label><label className="field">Shift Name<input value={openSessionForm.shift_name} onChange={(e) => setOpenSessionForm((prev) => ({ ...prev, shift_name: e.target.value }))} /></label><label className="field">Opening Float<input type="number" step="0.01" value={openSessionForm.opening_float} onChange={(e) => setOpenSessionForm((prev) => ({ ...prev, opening_float: e.target.value }))} /></label><label className="field" style={{ gridColumn: '1 / -1' }}>Note<textarea value={openSessionForm.opening_note} onChange={(e) => setOpenSessionForm((prev) => ({ ...prev, opening_note: e.target.value }))} /></label><div className="row"><button className="primary" type="submit">Open Session</button></div></form></section>}
      {currentSessionIsStale && <section className="stale-session-interlock" role="alert"><div><span className="eyebrow">Shift action required</span><h2>This register is still on {currentSession.business_date}</h2><p>Today is {todayISO()}. Close and reconcile this session before taking another order so sales, receipts, and reports use the correct business date.</p></div><div className="row wrap"><button type="button" className="primary" onClick={openQuickClose}>Count and close session</button><Link href="/sessions" className="secondary-link">Review session history</Link></div></section>}
      {sessions.length > 0 && !currentSessionIsStale && terminalScreen === 'spaces' && (
      ...
      {sessions.length > 0 && !currentSessionIsStale && terminalScreen === 'order' && (
        <section className="pos-redesign order-terminal-screen">
          ...
          <div className="order-workspace">
            ...
            <section className="order-items-panel" role="region" aria-label="Menu items" tabIndex={0}>
              ...
              <div className="order-item-grid">{productGroups.map((group, index) => <button key={group.key} type="button" className={`order-item-card ${index < 6 && search ? 'search-priority-card' : ''} ${group.photo_url ? 'has-photo' : 'no-photo'} ${!group.is_available ? 'sold-out' : ''}`} disabled={!group.is_available} onClick={() => chooseProductGroup(group)}>{!group.is_available && <span className="sold-out-ribbon">Sold Out</span>}{!group.representative?.external_sku_id && <span className="local-only-ribbon">Local only</span>}{group.photo_url ? <img src={group.photo_url} alt="" loading="lazy" onError={(event) => { event.currentTarget.remove(); }} /> : <span className="item-photo-fallback">{String(group.label || '?').trim().slice(0, 1).toUpperCase()}</span>}<span className="item-card-body"><strong>{group.label}</strong><span className="item-card-meta"><b>{groupPriceLabel(group)}</b>{!group.is_available ? 'Sold out' : group.availableItems.length > 1 ? `${group.availableItems.length} choices` : group.has_options ? 'Customize' : 'Tap to add'}</span></span></button>)}{!productGroups.length && <div className="card muted">No active items in this category.</div>}</div>
            </section>
