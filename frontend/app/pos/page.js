...
export default function PosPage() {
  ...
  async function handlePay() {
    ...
    try {
      ...
      setReceiptPreview(settled); saveLastReceipt(settled, { ownerId: user?.id, registerId: currentSession?.register_id }); setLastReceipt(settled); setPaymentOpen(false); setNotice(settled.status === 'folio_pending' ? `Order ${order.order_no} finalized to pending folio.` : `Order ${order.order_no} paid.`); clearCart(); setTerminalScreen('spaces'); await loadAll({ silent: true });
